...
import { X, UploadCloud, FileVideo, BookOpen, Hammer, Popcorn } from 'lucide-react';
...
export const VideoUploadModal: React.FC<VideoUploadModalProps> = ({ isOpen, onClose, onUploaded }) => {
  ...
  const categories: Category[] = ['HLD', 'DSA', 'AI', 'Java', 'Cybersecurity', 'Cloud', 'Hardware', 'Career', 'WebDev', 'DevOps'];
  const formats: Format[] = ['Explainer', 'Tutorial', 'Comparison', 'Meme', 'Vlog', 'News', 'Timepass', 'Study', 'Building'];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-[#FFFFFF] rounded-[10px] border border-[#E4E7EC] p-4 sm:p-6 relative my-auto space-y-4 shadow-2xl">
        <button onClick={onClose} className="absolute top-3.5 right-3.5 p-1.5 rounded-[4px] text-slate-400 hover:text-[#12172B] active:scale-95"><X className="w-4 h-4" /></button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className={`py-2 px-3 rounded-[6px] border text-xs font-medium flex items-center justify-center gap-1.5 transition-all ${
                  pillar === 'Study'
              ...
              >
                <Hammer className="w-3.5 h-3.5" /> Building / Project
              </button>
              <button
                type="button"
              ...
              >
                <Popcorn className="w-3.5 h-3.5" /> Timepass / Meme
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" placeholder={pillar === 'Timepass' ? "e.g. When the code compiles on first try 😂" : "e.g. Raft Consensus Algorithm in 60 Seconds"} value={title} onChange={e => setTitle(e.target.value)} className="w-full px-3 py-2 rounded-[6px] border border-[#E4E7EC] text-xs focus:outline-none focus:border-[#3654FF]" required />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value as Category)} className="w-full px-2 py-2 rounded-[6px] border border-[#E4E7EC] text-xs font-mono focus:outline-none focus:border-[#3654FF]">
                {categories.map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium block mb-1">Difficulty</label>
              <select value={difficulty} onChange={e => setDifficulty(e.target.value as Difficulty)} className="w-full px-2 py-2 rounded-[6px] border border-[#E4E7EC] text-xs font-mono focus:outline-none focus:border-[#3654FF]">
                {difficulties.map(d => <option key={d}>{d}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium block mb-1">Format</label>
              <select value={format} onChange={e => setFormat(e.target.value as Format)} className="w-full px-2 py-2 rounded-[6px] border border-[#E4E7EC] text-xs font-mono focus:outline-none focus:border-[#3654FF]">
          ...
          <div>
            <div className="flex rounded-[6px] bg-[#F7F8FA] p-1 border border-[#E4E7EC] mb-3">
              <button type="button" onClick={() => setUseDirectUrl(false)} className={`flex-1 py-1.5 text-xs font-medium rounded-[4px] ${!useDirectUrl ? 'bg-white text-[#12172B] shadow-sm' : 'text-slate-500'}`}>
                Upload Video File
              </button>
              <button type="button" onClick={() => setUseDirectUrl(true)} className={`flex-1 py-1.5 text-xs font-medium rounded-[4px] ${useDirectUrl ? 'bg-white text-[#12172B] shadow-sm' : 'text-slate-500'}`}>
                Paste Direct URL
              </button>
            </div>

            {useDirectUrl ? (
              <input type="url" placeholder="https://example.com/video.mp4" value={directUrl} onChange={e => setDirectUrl(e.target.value)} className="w-full px-3 py-2 rounded-[6px] border border-[#E4E7EC] text-xs font-mono focus:outline-none focus:border-[#3654FF]" />
            ) : (
              <div
                onClick={() => fileInputRef.current?.click()}
                className="w-full p-6 rounded-[6px] border-2 border-dashed border-[#E4E7EC] hover:border-[#3654FF] cursor-pointer text-center transition-colors"
              >
                <input ref={fileInputRef} type="file" accept="video/*" className="hidden" onChange={e => { if (e.target.files?.[0]) setVideoFile(e.target.files[0]); }} />
                {videoFile ? (
                  <div className="flex items-center justify-center gap-2 text-xs text-[#12172B]">
                    ...
                    <span className="text-slate-400">({(videoFile.size / 1024 / 1024).toFixed(1)} MB)</span>
                  </div>
                ) : (
                  <div>
          ...
          <div className="grid grid-cols-2 gap-4 p-3 rounded-[6px] bg-[#F7F8FA] border border-[#E4E7EC]">
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-[#0F9C93] font-semibold">Educational Value</span><span className="font-bold">{educationalValue}</span>
              </div>
              <input type="range" min="0" max="100" value={educationalValue} onChange={e => setEducationalValue(+e.target.value)} className="w-full accent-[#0F9C93]" />
            </div>
            <div>
              <div className="flex justify-between text-xs font-mono mb-1">
                <span className="text-[#C98A2C] font-semibold">Hype / Entertainment</span><span className="font-bold">{hypeScore}</span>
              </div>
              <input type="range" min="0" max="100" value={hypeScore} onChange={e => setHypeScore(+e.target.value)} className="w-full accent-[#C98A2C]" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea rows={2} placeholder="Key concepts or punchline for semantic ranking..." value={transcript} onChange={e => setTranscript(e.target.value)} className="w-full px-3 py-2 rounded-[6px] border border-[#E4E7EC] text-xs font-mono focus:outline-none focus:border-[#3654FF]" />
          </div>

          <button type="submit" disabled={uploading} className="btn-primary w-full py-2.5 text-xs flex items-center justify-center gap-2">
